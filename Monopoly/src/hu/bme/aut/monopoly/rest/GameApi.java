...
public class GameApi {
	...
	public Response getActiveGames(@Context HttpServletRequest request) {
		...
		List<Game> activeGamesByEmail = mem
				.getActiveGamesByEmail(loggedInUseremail);
		...
		for (Game game : activeGamesByEmail) {
			...
			try {
				...
				aActiveGame.put("actualPlayer", game.getActualPlayer()
						.getUser().getName());
				...
				for (Player player : game.getPlayers()) {
					System.out.println("JATEKOS NEV: " + player.getId() + " - "
				...
				return Response.status(Response.Status.NO_CONTENT)
						.entity("Can not create JSON.").build();
			}
			activeGamesJsonArray.put(aActiveGame);
		}
		mem.closeDB();
		...
		return Response.ok(responseJsonObject.toString(),
	...
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	public Response openGame(String json, @Context HttpServletRequest request) {
		String loggedInUseremail = Helper.getLoggedInUserEmail(request);
		...
		try {
			...
			return Response.status(Response.Status.UNSUPPORTED_MEDIA_TYPE)
					.entity("Invalid JSON: "+json).build();
		}
		MonopolyEntityManager mem = new MonopolyEntityManager();
		mem.initDB();
		...
		try {
			...
			gameDetailesJsonObject.put("ownerOfGame", game.getOwnerOfGame()
					.getId());
			gameDetailesJsonObject.put("actualPlayer",
					getAllDetailesOfPlayer(game.getActualPlayer()));
			...
			gameDetailesJsonObject.put("acceptedPlayers",
					acceptedPlayersJsonArray);
			...
			for (Place place : game.getPlaces()) {
				...
				aPlace.put("placeSequenceNumber",
						place.getPlaceSequenceNumber());
				...
				for (Player player : game.getPlayers()) {
					if (player.getSteps().get(player.getSteps().size() - 1)
							.getFinishPlace() == place) {
						JSONObject aPlayerOnPlaceJsonObject = new JSONObject();
						aPlayerOnPlaceJsonObject
						.put("playerId", player.getId());
						aPlayerOnPlaceJsonObject.put("userName", player
								.getUser().getName());
		...
		return Response.ok(gameDetailesJsonObject.toString(),
				MediaType.APPLICATION_JSON).build();
	}

	private JSONObject getAllDetailesOfPlayer(Player player)
		...
		aPlayerJsonObject.put("placeSequenceNumber",
				player.getSteps().get(player.getSteps().size() - 1)
		...
		for (BuildingPlace buildingPlace : player.getBuildings()) {
			...
			aOwnedBuildingJsonObject.put("buildingId", buildingPlace
					.getBuilding().getId());
			aOwnedBuildingJsonObject.put("buildingName", buildingPlace
					.getBuilding().getName());
			aOwnedBuildingJsonObject.put("nuberOfHouse",
					buildingPlace.getHouseNumber());
			aOwnedBuildingJsonObject.put("price", buildingPlace.getBuilding()
					.getPrice());
			aOwnedBuildingJsonObject.put("housePrice", buildingPlace
					.getBuilding().getHousePrice());
			aOwnedBuildingJsonObject.put("baseNightPayment", buildingPlace
					.getBuilding().getBaseNightPayment());
			aOwnedBuildingJsonObject.put("perHousePayment", buildingPlace
					.getBuilding().getPerHousePayment());
			aOwnedBuildingJsonObject.put("maxHouseNumber",
					5 - buildingPlace.getHouseNumber());
	...
	private boolean isPlayerActualPlayerOfTheGame(Game game,
			boolean isActualPlayer, User user) {
		for (Player playerOfUser : user.getGamePlayers()) {
			if (playerOfUser.getGame() == game
					&& (playerOfUser == game.getActualPlayer())) {
				isActualPlayer = true;
			}
		}
	...
	private JSONObject getGameDetailes(Game game) throws JSONException {
		...
		aGameJsonObject.put("notAcceptedYetPlayers",
				notAcceptedYetPlayersJsonArray);
	...
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	public Response getBuilding(String json, @Context HttpServletRequest request) {
		// TODO minek ez?
		String loggedInUseremail = Helper.getLoggedInUserEmail(request);
		JSONArray jsonTomb;
		...
		try {
			jsonTomb = new JSONArray(json);
			placeSequenceNumber = jsonTomb.getJSONObject(0).getInt(
					"placeSequenceNumber");
			System.out.println("BuildigPlaceSequenceNumber: "
					+ placeSequenceNumber);
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		if (mem.getPlaceByPlaceSequenceNumber(placeSequenceNumber) instanceof BuildingPlace) {
			BuildingPlace buildingPlace = mem
					.getBuildingPlaceByPlaceSequenceNumber(placeSequenceNumber);
			System.out.println("BuildigPlace: " + buildingPlace.getId() + " - "
					+ buildingPlace.getOwnerPlayer());
			if ((buildingPlace != null)
					&& (buildingPlace.getOwnerPlayer() != null)) {
				try {
					buildingPlaceJsonObject.put("ownerUserName", buildingPlace
							.getOwnerPlayer().getUser().getName());
					getBuildingPlaceDetailes(buildingPlace,
							buildingPlaceJsonObject);
				} catch (JSONException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
		...
		return Response.ok(buildingPlaceJsonObject.toString(),
				MediaType.APPLICATION_JSON).build();
	}

	private JSONObject getBuildingPlaceDetailes(BuildingPlace buildingPlace,
			JSONObject buildingPlaceJsonObject) throws JSONException {
		buildingPlaceJsonObject.put("houseNumber",
				buildingPlace.getHouseNumber());
		buildingPlaceJsonObject.put("name", buildingPlace.getBuilding()
				.getName());
		buildingPlaceJsonObject.put("price", buildingPlace.getBuilding()
				.getPrice());
		buildingPlaceJsonObject.put("housePrice", buildingPlace.getBuilding()
				.getHousePrice());
		buildingPlaceJsonObject.put("baseNightPayment", buildingPlace
				.getBuilding().getBaseNightPayment());
		buildingPlaceJsonObject.put("perHousePayment", buildingPlace
				.getBuilding().getPerHousePayment());
		buildingPlaceJsonObject.put("placeSequenceNumber",
				buildingPlace.getPlaceSequenceNumber());
	...
	public Response getInvitations(@Context HttpServletRequest request) {
		...
		for (Player player : gamePlayers) {
			if (player.getPlayerStatus() == PlayerStatus.notAcceptedYet) {
				try {
					notAcceptedYetGamesJsonArray.put(getGameDetailes(player
							.getGame()));
	...
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	public Response acceptInvitation(String json,
	...
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	public Response refuseInvitation(String json,
			@Context HttpServletRequest request) {
	...
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	public Response createGame(String json, @Context HttpServletRequest request) {
		boolean success = true;
		...
		try {
			...
			JSONArray playersJsonArray = jsonTomb.getJSONObject(0)
					.getJSONArray("players");
			for (int i = 0; i < playersJsonArray.length(); i++) {
				System.out.println(playersJsonArray.getJSONObject(i).getString(
						"player"));
				String playerEmailOrName = playersJsonArray.getJSONObject(i)
						.getString("player");
				...
				if ((mem.isUserNameRegistered(playerEmailOrName))) {
					...
					System.out.println("DB-USerName: "
							+ mem.getUserByName(playerEmailOrName).getName());
					// Player player = new Player();
				...
				// valami jott, de az a usernamek kozott nincs az adatbazisban
					...
					if (mem.isUserEmailRegistered(playerEmailOrName)) {
						...
						for (Player p : user.getGamePlayers()) {
							if(p.getPlayerStatus()==PlayerStatus.accepted || p.getPlayerStatus()==PlayerStatus.notAcceptedYet) {
								hasActiveGame = true;
							}
						}

						if (user.getUserType() == UserType.notRegistered && hasActiveGame) {
							notRegisteredUserEmailsJsonArray
							.put(playerEmailOrName);
							System.out.println("NOT REG TOMBBE: "
									+ playerEmailOrName);
							errorCode = 1;
						} else {
							validUsers.add(user);
							System.out.println("DB-Email: "
									+ mem.getUserByEmail(playerEmailOrName)
									.getEmail());
						}
					}
					...
					// ami nincsen az adatbazisban
						...
						if (playerEmailOrName.contains("@")) {
							...
							// validUsers.add(mem.getUserByName(playerEmailOrName));
							validUsers.add(mem
									.getUserByEmail(playerEmailOrName));
							System.out.println("NO-DB-Email: "
									+ mem.getUserByEmail(playerEmailOrName)
									.getEmail());
							inviteUsers.add(mem
									.getUserByEmail(playerEmailOrName));
						...
						// nem regisztralt username
							...
							System.out.println("INVALID TÖMBBE: "
									+ playerEmailOrName);
			...
			if (errorCode == 0) {
				...
				try {
					...
					for (User user : inviteUsers) {
						System.out.println("Invitation kuldese: "
								+ user.getEmail());
		...
		try {
			...
			responseJsonObject.put("invalidUserNames",
					invalidUserNamesJsonArray);
			responseJsonObject.put("notRegisteredUserEmails",
					notRegisteredUserEmailsJsonArray);
	...
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	public Response getProfil(@Context HttpServletRequest request) {
		String loggedInUseremail = Helper.getLoggedInUserEmail(request);
		JSONObject responseJsonObject = new JSONObject();
		...
		int invitationsNum = getNumberOfGameInAStatus(user,
				PlayerStatus.notAcceptedYet);
		...
		try {
			...
			responseJsonObject.put("participatedGamesNum", user
					.getGamePlayers().size());
	...
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	public Response getPlaceData(String json,
			@Context HttpServletRequest request) {
		System.out.println(json);
		JSONArray jsonTomb;
		int placeSequenceNumber = 0;
		try {
			...
			placeSequenceNumber = jsonTomb.getJSONObject(0).getInt(
					"placeSequenceNumber");
			System.out.println("PlacePlaceSequenceNumber: "
					+ placeSequenceNumber);
		} catch (JSONException e) {
			e.printStackTrace();
			return Response.status(Response.Status.UNSUPPORTED_MEDIA_TYPE)
		...
		try {
			if (place instanceof StartPlace) {
				StartPlace startPlace = mem
						.getStartPlaceByPlaceSequenceNumber(placeSequenceNumber);

				placeJsonObject.put("placeId", startPlace.getId());
				placeJsonObject.put("placeSequenceNumber",
						startPlace.getPlaceSequenceNumber());
				placeJsonObject.put("throughMoney",
						startPlace.getThroughMoney());
				...
				placeJsonObject.put("placeSequenceNumber",
						buildingPlace.getPlaceSequenceNumber());
				placeJsonObject.put("houseNumber",
						buildingPlace.getHouseNumber());
				if (buildingPlace.getOwnerPlayer() != null) {
					placeJsonObject.put("ownerPlayerId", buildingPlace
							.getOwnerPlayer().getId());
				} else {
					placeJsonObject.put("ownerPlayerId", "");
				}
				if (buildingPlace.getBuilding() != null) {
					placeJsonObject.put("buildingId", buildingPlace
							.getBuilding().getId());
					placeJsonObject.put("buildingName", buildingPlace
							.getBuilding().getName());
					placeJsonObject.put("buildingPrice", buildingPlace
							.getBuilding().getPrice());
					placeJsonObject.put("buildingHousePrice", buildingPlace
							.getBuilding().getHousePrice());
					placeJsonObject.put("buildingBaseNightPayment",
							buildingPlace.getBuilding().getBaseNightPayment());
					placeJsonObject.put("buildingPerHousePayment",
	...
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	public Response getPlayerData(String json,
			@Context HttpServletRequest request) {
		System.out.println("JSON: " + json);
		JSONArray jsonTomb;
		int playerId = 0;
		...
		try {
			...
			playerJsonObject.put("playerSequenceNumber", player.getGame()
			...
			for (BuildingPlace buildingPlace : player.getBuildings()) {
				...
				ownedBuildingsJsonArray.put(getBuildingPlaceDetailes(
						buildingPlace, buildingPlaceJsonObject));
			}

			playerJsonObject
			.put("ownedBuildingPlaces", ownedBuildingsJsonArray);
		} catch (JSONException e) {
			e.printStackTrace();
			return Response.status(Response.Status.NO_CONTENT)
					.entity("Can not create JSON.").build();
		...
		return Response.ok(playerJsonObject.toString(),
				MediaType.APPLICATION_JSON).build();
	}
	...
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	public Response makeStep(String json, @Context HttpServletRequest request) {
		int errorCode = 0;
		...
		try {
			...
			isBuildingBought = jsonTomb.getJSONObject(0).getBoolean(
					"isBuildingBought");
			isPayed = jsonTomb.getJSONObject(0).getBoolean("isPayed");
			isSold = jsonTomb.getJSONObject(0).getBoolean("isSold");
			soldBuildingsIds = jsonTomb.getJSONObject(0).getJSONArray(
					"soldBuildingsIds");
			boughtHouseNumberForBuildings = jsonTomb.getJSONObject(0)
					.getJSONArray("boughtHouseNumberForBuildings");
		...
		if (roll < 1
				|| roll > 6
				|| ((player.getSteps().get(player.getSteps().size() - 1)
						.getFinishPlace().getPlaceSequenceNumber() + roll) % 16 != placeSequenceNumber)) {
			errorCode = 2;
			...
			// leptetes
			step.setFinishPlace(mem
					.getPlaceByPlaceSequenceNumber(placeSequenceNumber));
			player.addStep(step);
			...
			try {
				...
				// buildingPlacere leptunk
					...
					if (isBuildingBought) {
						player.setMoney(player.getMoney()
								- buildingPlace.getBuilding().getPrice());
					...
					// fizetett a telek gazdájának megfelelõ összeget
						...
						int amount;
						amount = buildingPlace.getBuilding()
								.getBaseNightPayment()
								+ buildingPlace.getHouseNumber()
								* buildingPlace.getBuilding()
								.getPerHousePayment();

						ownerOfBuildingPlace = buildingPlace.getOwnerPlayer();
						ownerOfBuildingPlace.setMoney(ownerOfBuildingPlace
								.getMoney() + amount);
					...
					if (isSold) {
						...
						{
							soldBuildingPlaceSequenceNumber = soldBuildingsIds
							...
							player.setMoney(player.getMoney()
									+ ((int) (soldBuildingPlace.getBuilding()
											.getPrice() / 2)));
				...
				for (int i = 0; i < boughtHouseNumberForBuildings.length(); i++) {
					buildingId = boughtHouseNumberForBuildings.getJSONObject(i)
							.getInt("buildingId");
					number = boughtHouseNumberForBuildings.getJSONObject(i)
							.getInt("number");

					// LEELLENORIZNI H VAN_E ILYEN ID BUILDING
					if (mem.getBuildingPlaceById(buildingId) != null) {
						BuildingPlace boughtBuildingPlace = mem
								.getBuildingPlaceById(buildingId);
						...
						// telken
						int newSumHouseNumber = boughtBuildingPlace
						...
						player.setMoney(player.getMoney()
								- boughtBuildingPlace.getBuilding().getPrice()
						...
						errorCode = 2;
						throw new IllegalArgumentException(
								"No BuildingPlace with id : " + buildingId);
					}
			...
			try {
				...
				int nextActualPlayerIndex = (realPlayers.indexOf(player) + 1)
						% realPlayers.size();
				...
				EmailManager.sendStepSummaryEmail(
						realPlayers.get(nextActualPlayerIndex).getUser()
						.getEmail(), game);
			} catch (Exception e) {
				entityManager.getTransaction().rollback();
				e.printStackTrace();
		...
		if (errorCode == 0) {
			HttpSession session = request.getSession(true);
			String notLoggedInUseremail = (String) session
					.getAttribute("notLoggedInUser");
			if ((notLoggedInUseremail != null)
					&& !(notLoggedInUseremail.equals(""))) {
				session.invalidate();
			}
		}
